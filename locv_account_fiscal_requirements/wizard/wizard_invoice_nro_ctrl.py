...
class WizardInvoiceNroCtrl(models.TransientModel):
    ...
    date = fields.Date('Date',default=fields.Date.today,
                       help="Fecha utilizada para el papel dañado declarado.")
    ...
    nro_ctrl = fields.Char(
        ...
        help="Número utilizado para gestionar facturas preimpresas, por ley "
                "Necesito poner este número aquí para declarar"
                "Informes de impuestos correctamente.", store=True)
       ...
       # tax_obj = self.env['account.invoice.tax']
        ...
        if inv_brw.nro_ctrl:
            ...
            raise exceptions.except_orm(
                ...
                _("Puede ejecutar este proceso solo si la factura tiene Numero de Control, "
                  " verifique la factura e intente nuevamente."))
    ...
    def create_invoice(self):
        ...
        for wizard in wizard_brw:
            ...
            if (wizard.invoice_id and wizard.invoice_id.company_id.jour_id and
                ...
                raise exceptions.except_orm(
                    _('Error de Validación!'),
                    _("Debe ir al formulario de empresa y configurar un diario "
                      "y una cuenta para facturas dañadas"))
