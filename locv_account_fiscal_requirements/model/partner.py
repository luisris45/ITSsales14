...
class ResPartnerInherit(models.Model):
    ...
    def _check_partner_invoice_addr(self):
        ...
        context = self._context or {}
        if self.vat and self.vat[:2].upper() == 'VE' and \
                not self.parent_id:
            res = self.type == 'invoice'
            if res:
                return True
            else:
                return False
    ...
    def vat_change_fiscal_requirements(self, value):
        ...
        if res:
            ...
            return super(ResPartnerInherit, self).vat_change(self, value)
    ...
    def vies_vat_check(self, country_code, vat_number):
        ...
        if country_code.upper() != "VE":
            return super(ResPartnerInherit, self).vies_vat_check(country_code, vat_number)
        else:
            return super(ResPartnerInherit, self).simple_vat_check(country_code, vat_number)

    def update_rif(self):
        """ Load the rif and name of the partner from the database seniat
        """
        context = self._context or {}
        return self.env['seniat.url'].update_rif(self)
    ...
    def button_check_vat(self):
        ...
        super(ResPartnerInherit, self).check_vat(self)
        user_company = self.env['res.users'].browse(self).company_id
